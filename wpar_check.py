...
def get_data(filt, wtype, domask=False):
    ...
    instead. If wtype='DR' a random map is made and x,y positions are from catalog.
    '''
    ...
    if wtype == 'RR' or wtype == 'DR':
        ...
        if filt != 'check':
            ...
            # Check map. Replace ones in map with catalog positions if wtype = RR. 
            # Or else just use the galaxy catalog.
            im *= 0.
            im[randx, randy] = 1.0
    ...
    xy = np.dstack((x,y))[0,0:] 
    data = ({'im':im, 'hd':hd, 'xy':xy, 'nbins': 0, 'pscale':pscale, 'fname':fname})
...
def wsum(xy, shared_im, rbins, rbins_edges, savename, index, tic): 
    ...
    # If this loop has already been done, skip this iteration. Useful for stopping and starting.
...
#        return
    ...
    nrad = len(rbins)    
    wtheta = ({'sum':np.zeros(nrad), 'r':np.zeros(nrad)})# 'n':np.zeros(nrad), 'nonz':np.zeros(nrad),
    ...
    for irad in range(0, nrad*2, 2):
        ...
        if region.size > 0:            
            ...
            #wtheta['n'][j] = region.size
            #wtheta['nonz'][j] = len(np.where(region != 0)[0])
            #wtheta['mean'][j] = region.mean()
    ...
    try:
        ...
        if hostname != 'elgordo':
            try:
                ...
                os.system('mv '+pdir + '*.pickle ' + thomedir)
                print('Removing '+pdir)
                shutil.rmtree(pdir)
