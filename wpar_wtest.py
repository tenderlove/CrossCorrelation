...
def get_info(filt, *args):
    ...
    if filt == 'plw':
        ...
        pscale = 30.
        fname = '/home/ketron/cross/check/sdss_check_map.fits'
    else:
    ...
    d1 = hdu[exten].data.shape[0]
    d2 = hdu[exten].data.shape[1]
    if d1 > d2:
        increase = d1 - d2
    ...
    p.writeto('im.fits',clobber=True)
    print('Wrote im.fits')
...
def wsum(xy, shared_xgrid, shared_ygrid, shared_im, shared_mask, rbins, rbins_edges, savename, index, tic): 
    ...
    mapval = shared_im[x,y]
    #print 'x,y = ',x,y
    #print'im[x,y] = ',mapval #if this doesn't = 1 with -check then something's fucked.
    #print 'im[x+1,y] = ',shared_im[x+1,y]
    # Radial coords
    r = abs( (shared_xgrid - x) + 1j*(shared_ygrid - y) ) 
    nrad = len(rbins)    

    wtheta = ({'sum':np.zeros(nrad), 'numel':np.zeros(nrad), 'r':np.zeros(nrad),
               'mean':np.zeros(nrad), 'nonz':np.zeros(nrad)})#, 'std':np.zeros(nrad)}) 
                #'mean':np.zeros(nrad)

    ##plt.ion()
    ##plt.figure()
    ##plt.imshow(shared_im)
    #shared_im = shared_im.copy()
    #shared_im = np.asarray(shared_im)
    j = 0
    plotted = False
    for irad in range(0, nrad*2, 2):
        ...
        if not thisindex.ravel().any():
            ...
            wtheta['mean'][j] = np.mean(shared_im[thisindex][nonzindex])            
            ...
            #im[x,y] = 5
            #zinds = np.where(thisindex == 0)
            #thisindex[zinds] = -1
            #im *= thisindex
            #p = pyfits.PrimaryHDU(im)
            #p.writeto('/home/ketron/cross/check/im_'+str(index)+str(j)+'.fits',clobber=True)
            #print('wrote /home/ketron/cross/check/im_'+str(index)+str(j)+'.fits')
            #print 'x,y = ',x,y
            ##print 'minmax(rad) = ',minrad, maxrad
            ##print 'sum = ', wtheta['sum'][j]
            ##print 'numel = ',wtheta['numel'][j]
            ##print 'mean = ',wtheta['mean'][j]
...
if __name__ == '__main__':
    ...
    if wtype != 'DD' and wtype != 'RR':
        print('You need to choose either "RR" or "DD" for wtype.')
    ...
    cr = os.getenv('cr')
    pdir = cr + 'pickle/pickle_' + catname+'_'+Hfilter  # pickle save directory/name.
    ...
    if not testrun: 
        ...
        if catname == 'nvss':
            ...
            print('/data/users/ketron/nvss_cat.fits')
            ...
            #hdu = pyfits.open('/home/ketron/cross/check/sdss_DR12_partial_check.fits')
            #ra = hdu[1].data['ra']
            #dec = hdu[1].data['dec']
            #radec = np.dstack((ra,dec))[0,0:]
            #xy = info['hd'].wcs_world2pix(radec, 1)
    ...
    info['nbins'] = nbins # set to keyword value!
    ...
    if Linear: 
        ...
        min, max = 1., 2.*min(im.shape) #m.floor(m.sqrt(dim1**2 + dim2**2))
        ...
        radial = 10 ** np.linspace(np.log10(min), np.log10(max), info['nbins']+2)
        nrad = len(radial)
